...
**eligible = {v028 koff 888.2, v029 koff 902.1}＝両枠とも健全ウォーカー（固定点koff≈915、cut近傍）**。
締切8/16（Simulation）・9/13（Strategy）、最終提出目標8/2。5330チーム。
...
### 07-20の決着（詳細: daily_reports/20260720.md）
- **exp065（エネルギー基盤仮説）step 0で棄却**: 「特殊エネ8枚=E.Hammerの餌」→ 実測で**逆相関**
  （Hammerは我々の勝ちと相関、EH≥2でwr 0.64）。負けの正体はアタッカーKOサイクル（構造的）。
  レバー測定ファーストが10分で足切り、計算投資ゼロ。
- **外部採用スイープ初回（新LB#1 LumenLiquidity）**: Dragapult ex/Dusknoir型を抽出・専用パイロットで
  候補評価**0.581 < 基準0.786 → 不採用**（Alakazamキラーだが archaludon/lucario/wall に穴だらけ）。
