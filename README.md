...
**eligible = {v013 reply-guard, v012 deck-ratio}**（~800帯）。**v013 = 初の本番探索層**（相手応手を engine 探索で読む doom 拒否権, ex_lucario 0.77→0.86 有意）。
**07-01 に運営がエンジンソース公開** → ネイティブビルド（g++ 15秒, 公式互換）→ **20万試合の自己対戦で「中盤価値は学習可能(AUC 0.806)」を実証し exp014 を更新**。
ただし価値の実戦統合（exp033）は doom 拒否権に及ばず＝「**学習可能性はデータ律速・変換可能性は統合律速**」が確定（レポートの中心素材）。
**v013 敗因の53% = Dragapult + Archaludon の構造マッチ**。piloting の残レバー＝ターン内順序付け（exp035 turn-beam, 検証付き上書き, 評価中）。
...
| exp035 | **turn-beam**（ターン全系列のエンジン探索、プライズ+ダメージの全K検証付き上書き） | 常時上書き版は大退行(0.30)→検証付きに再設計（paired 0.50 無害・発火2.3/試合）。**n=200 評価中** |

### 提出（eligible = 最新2, 2026-07-04）
| 版 | 中身 | 状態 |
|---|---|---|
| **v013 reply-guard** | v012 + 相手応手ガード（belief-search Stage 1, doom 拒否権） | 提出済（sub 54269701, ~800-850 収束中）。ラダー勝率 0.52, 対 Lucario 0.88 |
| **v012 deck-ratio** | v_trev（Trevenant×4）+ v011 revenge | ~763-799。ペアの安全側 |
