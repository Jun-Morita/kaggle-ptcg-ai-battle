...
LB = **851.6**、**613位 / 6,822チーム**で silver cut 911.4 に **59.8 届いていない**。
締切は 8/16（Simulation、あと1日）と 9/13（Strategy、あと29日）です。

**cut を下回っているのは意図的です。**921.5 だった v060 を自分で押し出しました。
`competition/overview.md:39` に「締切後 約2週間継続対戦して LB 確定」とあり、
いま見えている数字は収束途中の通過点だからです。詳細は下記 08-15 の節。
