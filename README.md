...
**eligible = {v028 koff (810, 平坦化), v029 koff (888.4@day1=過去2番目の初動, 新エンジンビルド)}**。
silver cut **920.0**・bronze 867.2・5239チーム（silver=261位以内）。
締切8/16（Simulation）・9/13（Strategy）、最終提出目標8/2。
koff引き履歴: 942/708/745/759/592/810/**888**——**早期判定ルール**（day1-2で<800なら引き直し、
公式マッチメーカー仕様disc726690の経路依存ロックイン）でリロール高回転中。
**エンジン更新（disc727094、3行のクラッシュ悪用穴修正）に即応**: 新cgでbuild_koff2再ビルド・
検証済み、v029から採用。
...
### 進行中: exp060 = pub1034の69ノブ結合最適化（sNES、RL再検討の候補A）— 16/40世代
