...
eligible = **{v046 740.4, v047（v045のコピー, μ600から収束中）}**。silver cut は **#296 = 915.8 / 5936チーム**。締切8/16（Simulation）・9/13（Strategy）。

**今日、ローカル評価がラダーを外していた真因が分かった。予測できなかったのではなく、測る対象が違っていた。** 全ての net ゲートが「両席とも同じ Grimmsnarl」の**自己ミラー**で、「ミラーが強くなったか」しか測っていなかった。15日コーパスは **46%がミラー**なので、**ミラー特化・フィールド劣化はこの設計では原理的に観測できない**。v046 はこのゲートを 0.608 (z=+4.30, n=400) で通り、実ラダーで v045 に **146点負けた**（740.4 vs 889.9、全体勝率は両方 0.537 で同一＝より弱い相手に同じ勝率）。
