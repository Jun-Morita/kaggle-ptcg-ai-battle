...
### piloting強化レーンを「上限の証明つき」で閉鎖（07-21, 26件目）
ユーザー指摘を起点に3段階で追い込み、**オラクル（完璧な相手デッキ知識）でも −0.0075**
＝**認識をどれだけ精緻化しても勝率は上がらない**ことを実測。詳細は下記07-21節。

### 07-21の決着: exp070 述語スキャン → pilotingレーン閉鎖（詳細: daily_reports/20260721.md）
...
- **段階6（定性レビュー）**: 死んだコード（`active_is_ready_crustle` 参照ゼロ）、
  ...
  **メタ配分ミスマッチ**（`facing_lucario_strong` 32箇所/シェア6.7% vs `facing_alakazam` 1箇所/**41.3%**）、
  脅威モデルが粗い（エネ加速を無視、最安の技で「攻撃可能」を判定）。

**結論**: 識別可能な欠陥はどれも**0.01前後**（オラクル-0.0075／述語+0.007／探索+0.025／重み平坦）。
根本理由は exp067 の**単一クロック機構** — koffの勝ち筋はミル一本で相手が誰でもやることが変わらない。
**行動レパートリーが1本のデッキでは、認識を精緻化しても行き先がない。**
