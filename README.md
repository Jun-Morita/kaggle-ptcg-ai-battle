...
**eligible = {v026 koff (766.9→上昇中, 61試合0.557), v027 koff新ドロー (PENDING)} ＝ koff両建て**。
silver cut **919.4**（930.2から低下・追い風）・bronze 869.3・5188チーム（silver=259位以内）。
締切8/16（Simulation）・9/13（Strategy）、最終提出目標8/2（リロール機会あと~5回）。
...
- 監視中の穴: ライブでmixed_ex3に3-9（シェア14→20%）。実体はStarmie（0-3）/Iono/実Froslass変種
  （ローカルプール未収録）。Star-mineの実60枚を複製し床代理プローブで定量化中（exp054-F）。
