...
**「学習レーンは 24 ネガティブで終了」という結論の前提が誤っていたと判明し、一度開いた。ただし出荷段階で塞がった**。eligible = **{v043 BC-d128ctl, v041 alakazam}**（v040 koff 845.4 を退出）。rank ~#626 / 5718、silver cut ~917。締切8/16（Simulation）・9/13（Strategy）。

**今日の最大の学び（耐久性あり）**: **提出 sandbox では numpy を使わない・巨大アーティファクトを送らない**。同じ `"Validation Episode failed."` で v015（07-09）と v042（07-26）の2度死んでいる。v042 の検証エピソードは `remainingOverageTime` 600→598.674＝**エージェント時間の消費わずか1.33秒でプロセス即死**＝タイムアウトではなく、`try/except` では守れない（ネイティブ abort / OOM kill は例外にならない）。**ローカルの `sandbox_replica.py` は「numpy がある環境」しか再現できず、この失敗を構造的に検出できない**——緑のローカル検証は根拠にならなかった。
