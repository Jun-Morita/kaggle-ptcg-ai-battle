...
eligible = **{v053-gbdt-v4b 860.2, v054-gbdt-v6（提出直後）}**。553位 / 6,543チームで、
silver（上位5%）の cut は 902.0 ——**あと約42点**。締切は 8/16（Simulation）と 9/13（Strategy）です。

**LB は eligible 2件の最大値**と見られる（現在 LB 860.2 = v053 の 860.2、v052 は 849.5）。
公式規定は「最新2提出のみ最終評価」としか書いていないので推定だが、これが正しければ
提出枠の操作では silver に届かず、必要なのは素の実力向上だけ。

ラダーのノイズは非常に大きい。disc733083 で他参加者が「同じエージェントを2回出したら4時間後に
**400点**離れていた」「2日置けば安定する」と報告しており、我々の v045/v047（バイト同一で
884.5 と 860.9）とも整合する。締切後も約2週間試合が続いてσが縮むため、**上振れを固定する
戦略は成立しない**——効くのは素の強さだけ。
