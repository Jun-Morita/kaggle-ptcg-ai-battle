...
exp036（GA による操縦定数進化）は n=200 厳密検証でチャンピオンが基準級に崩壊＝**打ち切り**（小サンプル過学習＋GAの上限）。
exp037（turn-beam 予算拡張）は合計 2.650≈2.670 で**頭打ちを確認**（設計の問題であって探索サイズの問題でない）。
exp038（depth=2 alpha-beta + 相手アーキタイプ模倣）は3ラウンドのデバッグで**実バグ11件を発見・修正**したが、
n=40の確定検証で**全マッチが基準以下・合計1.575 << v012基準2.45**＝**誠実なネガティブとして不採用**
（詳細 [`workspace/exp038_beam2ply/SESSION_NOTES.md`](workspace/exp038_beam2ply/SESSION_NOTES.md)）。
exp039（v014+v013ガード+exp038相手モデルの合成）はn=100でv014と統計的パリティ（field total 2.63 vs 2.67）
＝**DO NOT SHIP、v014維持**（詳細 [`workspace/exp039_guardopp/SESSION_NOTES.md`](workspace/exp039_guardopp/SESSION_NOTES.md)）。
**exp040（公式RL/MCTSサンプルへの回帰＋その後の全知見を注入）を開始・進行中**: exp004の2バグ
（プレースホルダ決定化／データ規模不足）を修正しても小規模では対プール勝率0.000のままだったが、
価値ネットのAUC診断（0.489→0.628）で「学習不能」ではなく「データ律速」の初期段階と判明。
リプレイバッファを追加し2000世代規模の長時間学習を実行中（詳細 [`workspace/exp040_mctsv2/SESSION_NOTES.md`](workspace/exp040_mctsv2/SESSION_NOTES.md)）。
...
| exp040 | **公式RL/MCTSサンプルへの回帰**（決定化バグ修正+ネイティブエンジン+teacher-assisted self-play+リプレイバッファ） | exp004の2バグ修正のみでは小規模で対プール0.000。価値AUC診断(0.489→0.628)で「データ律速」と判明、リプレイバッファ追加し2000世代規模で長時間学習**実行中**（未決着） |

### 提出（eligible = 最新2, 2026-07-04）
| 版 | 中身 | 状態 |
|---|---|---|
| **v014 turn-beam** | v013 + ターン全体プランナー（検証付き上書きでターン内系列をビーム展開） | 提出済（μ600収束中）。crustle 0.765→**0.905**(+0.14, n=200) / 合計2.67 |
| **v013 reply-guard** | v012 + 相手応手ガード（belief-search Stage 1, doom 拒否権） | 提出済（sub 54269701）。ラダー勝率 0.52, 対 Lucario 0.88。eligible 継続 |
