...
**デッキを乗り換えた（v033-alakazam）**。07-13 以来はじめてのアーキタイプ転換。
eligible = **{v032 koff 809.8, v033 alakazam 806.2}** ＝ アーキタイプ1枚ずつ。
rank **949 / 5570、LB 809.8**、silver cut 913.3（+103.5）/ bronze 852.9（+43.1）。締切8/16（Simulation）・9/13（Strategy）。

### 模倣学習（BC）レーンに着手 — 外部実績が裏付け（07-23）
ヒューリスティックは打ち止め（下記5仮説棄却）＝残る +103 は**操縦技量**の問題。これを直接埋める
唯一の手段が BC で、外部に具体的実績がある:
- **disc728071**: 2人が独立に「**純粋な模倣学習・約21k試合**（H200で3〜4h）で今のスコア到達」。
  我々の教師供給（公式日次リプレイから 3,802件/日）と規模一致。PPO from scratch(1e7〜1e8)とは別物。
- **disc724362**: 「**#1以外のトップは探索を使っていない**」＝ numpy 純argmax の BC ネットがトップ帯に実在
  ＝提出制約（探索なし）は上限にならない。
- **3人が独立に「同じ checkpoint でもデッキでスコア激変」**。→ **BC の教師デッキは上位帯最多の
  Grimmsnarl（mixed_ex3, 上位帯43%・教師1,604件/日）に確定**（Alakazam 28%/876件より多い）。
