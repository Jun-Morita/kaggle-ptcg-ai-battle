...
**silver圏内、ただしマージン +0.6点**。eligible = **{v028 koff 917.0（~267位）, v029 koff 907.1（~304位）}**、
LB=2枠のmaxなので現在**917.0**。**silver cut 916.4（269位）** / 5,385チーム。
締切8/16（Simulation）・9/13（Strategy）、最終提出目標8/2。

固定点~915の揺れがそのままcut近傍にあり、**運用は完全にナイフエッジ**（v028は前日923.9から-6.9）。
**両枠HOLD継続**（リロール条件 <820で24-36h停滞 は未発火）。

### 述語の逐一精査 → 正しいが有意でない修正を1つ発見（07-21）
ユーザー指摘「1つ1つ精査して着実に」を受け、**コード読解で候補列挙 → 実戦データで誤答率測定
→ 生存分だけ修正 → 効果測定**の手続きを確立。`opponent_ex_pressure` のベンチex過大評価
（実戦41.3%）を直した `bench_strict` は n=200で +0.0102 も、**n=600で +0.0072（z≈1.5, 有意未達）
＋archaludon符号反転** → **単独出荷せず保持**（束ねて有意になったら提出）。詳細は下記07-21節。
...
- **段階7（誤答率→修正→測定）**——ユーザー指摘「1つ1つ精査して着実に」。
  ...
  修正 `bench_strict`（ベンチexは既にエネ充足＝昇格1テンポのみ脅威）は n=200で +0.0102 も、
  **n=600確定で +0.0072（z≈1.5, 有意未達）＋archaludon符号反転**（+0.040→-0.028＝ノイズ兆候）。
  → **単独出荷しない（ゲート不通過）**。修正は実測41.3%の本物のバグに対し論理的に正で退行なし・
  クラッシュ0なので**保持**し、他の述語修正と束ねて合計が有意（+0.02〜0.03）になったら一括提出。

**暫定結論**: 識別可能な欠陥はどれも小さい（オラクル-0.0075／探索+0.025／重み平坦／bench_strict+0.007）。
根本の**単一クロック機構**（exp067）が天井を規定し続ける。ただし**「1つ1つ精査して正しくする」手法
自体は有効**（コード読解6候補を実測で5棄却、生存1つは正だが単独では有意未達）＝着実な積み上げの土台。
