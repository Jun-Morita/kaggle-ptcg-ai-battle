...
<!-- UPDATE BEFORE SUBMITTING: rank line in the Abstract, and the date on it. -->
...
410 of 6,807 on 2026-08-31, partway through the evaluation period, and that number
is one draw rather than a measurement. This report is mostly about the thing that
...
and Teal Mask Ogerpon ex's Myriad Leaf Shower does 30 plus 30 for each Energy on
...
exactly, which mattered more than volume. An earlier corpus that accepted any
Grimmsnarl variant reached a higher held-out accuracy (top-k 0.684 → 0.798) while
losing head-to-head against our own shipped build (0.510 → 0.315): it had
faithfully learned to pilot somebody else's deck. Fidelity to the wrong teacher is
worse than having less data.
...
than as card indices: Prize cards left on each side, how many Knock Outs still
close the game, Energy attached against Energy required, whether an attack is
lethal this turn, how many Prize cards our own Bench is offering.
...
term that rewards effort, and we spent the effort. The same term sits on the
...
rank in this report, ours included, is a sample from something that wide.
...
us to 0.193. Weighted by what we faced, our true win rate was 0.33.
...
distribution. Two self-referential metrics disagreeing, with nothing outside to
settle it.
...
pool of bots scored by relative rating, which is what the competitors above us
built:
[Abhyuday](https://www.kaggle.com/competitions/pokemon-tcg-ai-battle/discussion/735649),
top 30, estimated his true ladder rating to within ±30 in the half hour after
submitting. We built the statistically careful version of the wrong measurement.
...
handicap bolted on to it.

The alternative was available, and we had used it once: in July we noticed the #1
player abandon his archetype for this one, checked 1,000 of his replays, and
followed him. [Anil Ozturk](https://www.kaggle.com/competitions/pokemon-tcg-ai-battle/discussion/736121)
did the same in August, starting on Grimmsnarl for the reason we did and switching
after seeing 9 of the top 12 teams on Dragapult. No local evaluation was
involved.
...
shape as Ozturk's portfolio model fine-tuned into specialists. Three teams arrived
at it independently, and it is what we lacked: our clones were each trained from
scratch on their own thin corpus, which is why none could pilot anything. Ozturk's
value-head numbers also explain why lookahead made our agent weaker twice: a value
head trained by imitation has only seen expert positions, and search asks it to
judge positions outside them (−20 to −30 with a cloned value head, +50 to +70 with
an on-policy one).
...
loaded, require zero fallbacks, and require a minimum smoke win rate, since a build
losing 0–25 to a weak opponent is broken rather than weak.

The policy is deterministic, so repeated matches differ only in the deal, and every
experiment records its seed, git SHA and config hash. The shipped artifact makes
about 197 decisions per game, worst observed move 1.28 s, and has completed its
ladder games with no validation errors.
...
against opponents we did not choose. Everything else on the list was answered by
that instrument long before the game had a chance to answer it.
