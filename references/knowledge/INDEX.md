...
- [締切後の公開実装 0817 / 上位の評価基盤 0818](postdeadline_shares_0817.md) — **Abhyuday**(disc735649, top30): 多数ボット総当たり arena で相対ELO を継続較正し**提出後30分で実ELOを±30推定**。弱点デッキは「そのデッキを回せる強い教師を自作して対戦相手に加える」で **dragapult 25%→70%**。我々の固定6セルゲートは相手が実物とずれても検出できず1週間気づけなかった＝**評価基盤への投資が最大の差**／**thwaitsusa2**(disc735675): ローカル `episodeSteps` 既定1000万で可逆ループがハング、シード依存で1回のプローブでは検出不能／旧記載: **rin ichikawa**(disc735444, MIT): **rin ichikawa**(disc735444, MIT): 約100語の用語集＋**92件の設計判断ログ**。評価と統計の章に**事前登録とCRN**があり、我々が exp052 で CRN を実装しながら出荷判定に使わずゲート破綻に1週間気づけなかったのと対照的／**hosikusa**(GA notebook 08-17): **デッキ枚数とルールベース重み38個を同一探索空間で同時最適化**＝我々が一度も試さなかった路線で、「デッキ固定・方策だけ改良」という構造的限界（我々の対フィールド勝率0.33、複製デッキ5種すべて操れず）に直結／運用: 公式 Timeline(8/17) とホスト告知(8/16) が矛盾し提出できなかった参加者あり、締切後の -NaN は**評価期間が未開始**の可能性(disc735571)
