...
- [gold帯の戦略構造 0714](gold_band_scout_0714.md) — トップ8実測: Alakazam妨害型(#1)→壁→LO→Alakazamの三すくみ、LOは1100超に不在（プラトー950-1000）、TR SpidopsがSafeguard系全部のキラー
- [disc/code新着 0714](disc_code_sweep_0714.md) — 公式「10%ランダムマッチ」回答、TR Spidops=9位1146の実力デッキ（Safeguardメタ全体のキラー）、30k試合タイミング解析（トップもほぼ探索なし）
- [帯域別メタ(myso) 0716](band_meta_myso_0716.md) — チーム層別: 900-999はAlakazam43%/Archaludon3%（我々のプールのArch26%はstale）、1000+は壁増、Rock Fighting Energyのルール解説
- [マッチメーカー内部仕様 0717](matchmaker_mechanics_0717.md) — staff全文回答: 優先度=σ+staleness×レート倍率(最大~8x)・相手はガウス窓サンプリング（帯域メタの公式確認）・経路依存ループ（day1の引きが固定化）・**最終評価フェーズも試合継続**（幸運高止まりは洗われ得る→最終2枠はビルド強度重視）
- [エンジン更新+07-18メタ 0718](engine_update_0718.md) — staff修正=TR専用エネ捨て札の3行（クラッシュ悪用穴）・新cgでbuild_koff2再ビルド検証済み／pilkwang: **TR Spidops 1.4→12.3%急伸**（Alakazam/Crustle両狩り、トリガー到達・要band確認）、Starmie deck≠agent独立確認
- [ラダーA/Bテストの限界 0721](ladder_ab_testing_0721.md) — disc727695: 他チームの8提出統制実験。**バイト同一の提出が50点差**（873/923）、戦績1試合差で95点差＝最初の約15戦(高K)が着地を決める。「ラダーでのA/Bテストはほぼ無意味」「2体同時提出は良い戦術」＝**我々のランダムウォーク模型・リロール運用・局所ゲート依存を外部から裏付け**
- [上位帯はニューラル勢 0721](top_band_neural_0721.md) — disc727565: 提出サイズとレートに明確な相関（1MB=800-950 / 3-10MB=**990-1000+**）。我々は499KBの純ヒューリスティックで固定点~918。**我々のRLネガティブ群は「手法が無効」でなく「予算・実装が届かなかった」が正確**という反証寄り証拠（レポートで正直に扱う）
- [ポケモン名 英日対照](pokemon_names_ja.md) — エンジンは英語名を返すため、日本語情報源との突き合わせ用。推測で埋めない（未確認は未確認と記載）
- [Discussion intel スイープ 0723](discussion_intel_0723.md) — disc728071/717697(**BC/純RLで silver 到達の独立実例2件**・表現が最重要レバー・Jakeの「Dragapultだけ弱い」)、724362(#1以外は探索なし=純argmax可)、721338(Boss's Orders/Ultra Ballは言語化困難=BCの土俵)、713608(別チーム retrospective・**探索は提出時に使えるという彼らの未解決問を我々は解決済み**・最大シェアのマッチが全てを決める)、723591先攻(既出・v020でNO-GO済み)
- [Discussion intel 0731](discussion_intel_0731.md) — **公式配布データは上位帯偏重**（disc730497、我々の「教師データに Mega Lucario ex が0.09%」と独立に一致）／**パイロット強度を分離すると上位デッキは横並び**（disc730823 Sumi 41日209,772試合: Grimmsnarl 補正後50.9%、Garchomp 54.2→50.6%は誤りと著者訂正）／**先後差が大きい**（Grimmsnarl 先攻54.2% 後攻47.7%）／22アーキタイプのデッキリスト公開(disc730554)
- [Discussion / Code intel 0725](discussion_intel_0725.md) — **収束機構**(disc728695 Mando: σ早期収束＝序盤ロックイン＝reroll効く / disc728935 Addison: Bo1維持・締切後episode頻度増)＝dragapult×2 は正当。**エンジン更新**(disc728587: Ninetales×Amarys SIGSEGV 修正、我々の cg は07-18版だがdragapultデッキは660/1207無しで無害＝今はリビルド不要)。**クラッシュ回避**(disc728519: op.typeは生int・不正indexは即死＝ラッパー対処済)。**公開エージェント上限**: skarin dragapult 827.5 が現時点で最良の採用可能公開agent、超えるものは無し。ルールベース天井はrank2＝**dragapultパイロット改良に伸びしろ**（次実験=scout-top）。公開NB締切 Aug 2
- [Discussion intel 0903](discussion_intel_0903.md) — **Pokemon Elements利用の公式回答**(disc736603, Addison Howard 08-24): カード名/効果/アーキタイプ名の言及は許可、公式ビジュアライザSSも可。カード原画の複製/改変は未確認のグレーゾーン(我々のthumbnail.pngは要検討)。／**Ogerpon ex→Grimmsnarl 87.0%(n=69)**を独立データで再確認(disc737107 Di Cio、締切後23,313試合)。パイロット補正すると48.0%までほぼ互角に戻るが、**上位10%帯ではシェア<0.1%とほぼ消滅、下位60%では22.6%と最頻出**＝writeup "most-played archetype"の文言は時点を要明示。／TrueSkill分散は複数人が過小評価を指摘(disc737435)＝我々のsd=51主張の傍証。／kernel pokesom: Dragapult exのルールベース厳密リーサル解法(ばら撒き2^5全探索+資源予約)
- [締切後の公開実装 0817-0821](postdeadline_shares_0817.md) — **Anil Ozturk**(disc736121): Grimmsnarl を「リプレイ量最大」で選んだ点まで我々と同一、そこから**上位12チーム中9がDragapultと確認してデッキを乗り換え**＝我々が取らなかった分岐。クローンの弱さは**10アーキタイプ横断 portfolio モデルからの fine-tune** で解ける問題だった。探索は **BC value head で -20〜30 点 / PPO value head で +50〜70 点**＝我々の探索A/B 2件棄却の説明（模倣由来の価値は on-policy 外を評価できない）／**10%ランダムマッチ**(disc736361)は高レート帯で勝ち+0・負け-15の非対称／評価期間: 08-20 にマッチメイキング正常化、我々は 2日で 888→860.6・434位→535位／**213tubo(Preferred)**(disc735867): BC+PPO・デッキ別専用方策で **16.7M params×135M env steps / 117M params×1.68B env steps**。Abhyuday(top30)は 2M/6M。**我々の PPO 却下(08-08)は判断は正しいが理由づけが誤り**＝手法が効かないのではなく効く水準まで回すのに2桁以上の計算が要る／**Ilia Iliev**(disc735940): マルチデッキ BC で1000超→PPO に1か月で Crustle 900・Dipplin 800、**RL は何も足さなかった**。critic の二値報酬過学習を BC プレイアウト10回で平滑化＝**我々の exp052 CRN の使い道そのもの**／**TommyCyd**(disc735975): 8チェックポイント切替 Dragapult に**別のデッキリストを梱包して提出**＝我々の silent failure 6件と同型／**Abhyuday**(disc735649, top30): 多数ボット総当たり arena で相対ELO を継続較正し**提出後30分で実ELOを±30推定**。弱点デッキは「そのデッキを回せる強い教師を自作して対戦相手に加える」で **dragapult 25%→70%**。我々の固定6セルゲートは相手が実物とずれても検出できず1週間気づけなかった＝**評価基盤への投資が最大の差**／**thwaitsusa2**(disc735675): ローカル `episodeSteps` 既定1000万で可逆ループがハング、シード依存で1回のプローブでは検出不能／**rin ichikawa**(disc735444, MIT): 約100語の用語集＋**92件の設計判断ログ**。評価と統計の章に**事前登録とCRN**／**hosikusa**(GA notebook 08-17): **デッキ枚数とルールベース重み38個を同一探索空間で同時最適化**＝我々が一度も試さなかった路線／運用: 公式 Timeline(8/17) とホスト告知(8/16) が矛盾、締切後の -NaN は評価期間未開始の可能性(disc735571)、Hero's Cape はエンジンバグと公式認定(disc735766)、評価期間の試合消化が極端に遅い(disc735822)
