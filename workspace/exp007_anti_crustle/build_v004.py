"""Build v004: Crustle anti-ex control MIMIC (the top-ladder archetype) + crash-safety.
Deck = the real meta Crustle decklist (from replays); policy = generic lucario_v2
(plays Dwebble->Crustle, attacks 120, heals). Beats ex decks (Lucario 0.75,
Dragapult 1.00). Submitted alongside the v003 counter as the 2-deck meta play."""
...
CG=os.path.join(REPO,'data','sim_sample','cg'); BUILD=os.path.join(HERE,'build_v004')
CRUSTLE=json.load(open(os.path.join(HERE,'crustle_deck.json')))
SAFETY='''

# ===== crash-safety wrapper (v004) =====
