"""exp033 eval — value-guard vs the exp027 field and PAIRED vs v013 (guard) / v012.
MATCH=field|v013|v012  VG_MARGIN=0.10  Usage: uv run python eval_vg.py [n]"""
...
def main():
    ...
    print(f"# value_guard K={VG.K} margin={VG.MARGIN} mode={mode} n={n}")
    for nm, mk in field:
        t0 = time.time()
        st = run_gauntlet(cand, mk(), n_games=n, swap_sides=True)
        dt = time.time() - t0
        print(f"  {nm:11s} wr={st.winrate0:.3f} ({st.wins0}-{st.wins1}-{st.draws}) "
              f"err={st.errors0} [{dt/max(n,1):.2f}s/g] stats={VG.STATS}", flush=True)
