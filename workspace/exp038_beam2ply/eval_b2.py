"""exp038 eval — depth-2 minimax (beam2_policy) vs field / paired v012 / paired v014.
MATCH=field|v012|v014, ONLY=<name>. Usage: uv run python eval_b2.py [n]"""
...
def main():
    ...
    print(f"# beam2ply depth={B2.CFG.depth} opp_mode={B2.CFG.opp_mode} "
          f"branch={B2.CFG.branch_single} opp_branch={B2.CFG.opp_branch} K={B2.K} n={n}")
    for nm, mk in field:
        t0 = time.time()
        st = run_gauntlet(cand, mk(), n_games=n, swap_sides=True)
        dt = time.time() - t0
        print(f"  {nm:11s} wr={st.winrate0:.3f} ({st.wins0}-{st.wins1}-{st.draws}) "
              f"err={st.errors0} [{dt/max(n,1):.2f}s/g] stats={B2.STATS}", flush=True)
