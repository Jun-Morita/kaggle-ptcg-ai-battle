...
Stage 1-era script (deck=lucario_v2, exp004's original bar). Superseded for
Stage 2 by train_mcts.pool_eval() / --pool-eval-every, which uses the REAL
5-matchup field on the real shipped deck (charmq) -- kept here only for
quick lucario_v2-deck comparisons against exp004's original numbers.
...
def make_mcts_agent(deck, model, search_count, opp_deck=None):
    """`opp_deck`: the REAL opponent decklist for correct exclusion-sampling
    determinization (determinize.py) -- defaults to `deck` (mirror), which is
    only right for the "random"/"lucario_v2" opponents below that happen to
    share this script's hardcoded deck; dragapult/lucario_v1 run DIFFERENT
    decks, so this must be passed explicitly for those (bug found + fixed
    2026-07-05 during a pre-Stage-2 code review)."""
    def agent(obs_dict):
        obs = T.to_observation_class(obs_dict)
        if obs.select is None:
            return list(deck)
        sel, _ = T.mcts_agent(obs_dict, deck, model, search_count, opp_deck=opp_deck)
        return sel
...
def main():
    ...
    opponents = {
        "random": (make_random_agent_with_deck(deck, seed=0), deck),
        "dragapult": (make_policy_agent("dragapult"), DECKS["dragapult"]),
        "lucario_v1": (make_policy_agent("lucario_v1"), DECKS["lucario_v1"]),
        "lucario_v2": (make_policy_agent("lucario_v2"), DECKS["lucario_v2"]),
    }
    res, wrs = {}, []
    with torch.inference_mode():
        for name, (opp, opp_deck) in opponents.items():
            mcts = make_mcts_agent(deck, model, search_count, opp_deck=opp_deck)
