...
# exp070 (07-21): _ex_evolution_ancestor_names() collects every name that is
# transitively an ancestor of ANY ex in the pool (105 names). Basics that evolve
# into BOTH an ex and a non-ex line therefore assert "ex pressure" on sight.
# Ground truth over 303 real ladder games: this predicate fired on 80.3% of games
# where the opponent ran NO ex; Dunsparce alone caused 48 of the 61 false-positive
# games. Dunsparce is the core of the non-ex attacker deck -- our largest matchup
# (band weight 0.289) -- so the false alarm turned on wall_mode against a deck that
# has no ex threat at all, stalling the mill clock.
# Measured (n=600/matchup, true CRN, McNemar over discordant pairs):
#   alakazam_dun 0.810 -> 0.847 (+0.037, z=2.43); band-weighted +0.0132, z=+2.16;
#   6 of 8 matchups had ZERO discordant games (structurally unaffected); 0 errors.
EX_EVOLUTION_ANCESTORS = {n for n in EX_EVOLUTION_ANCESTORS if n != "Dunsparce"}
...
def _base_agent(obs_dict: dict, configuration=None) -> list[int]:
    try:
...
# exp054-B KO_OFF patch: disable the ko_mode race arithmetic entirely.
# Measured (n=300/matchup, CRN, 9-opponent both-band pool): silver band
# 0.734 -> 0.792 (+0.057), our band +0.020; archaludon 0.697 -> 0.843,
# marnie 0.753 -> 0.887, no matchup regressed beyond noise. The original
# should_ko_mode ignores opponent healing (Jumbo Ice Cream / Hero's Cape),
# so it abandons the protected mill plan for prize races it loses.
def should_ko_mode(me, opponent, state):
    return False
