"""Field-audited Great Tusk / Crustle control policy.

This is a transparent adaptation of SOUTA Sakurai's public 1208-rated
LibraryOut policy (linked in the companion notebook).  The only strategic
change is the evidence-backed Xerosic priority for a *confirmed* visible
Lucario line, preserving the parent policy in every other matchup.
"""
...
def confirmed_lucario_line(opponent) -> bool:
    """Require an actual Lucario-line card, not a shared trainer signature."""
    ids = {pokemon.id for pokemon in field_pokemon(opponent)}
    ids.update(card.id for card in (opponent.discard or []) if card is not None)
    return bool(ids & {673, 674, 675, 676, 677, 678})
...
def play_score(card_id: int, me, opponent, state, wall_mode: bool, ko_mode: bool) -> int:
    ...
    if card_id == EXPLORER_GUIDANCE:
        ...
        if not state.supporterPlayed and active_ready_tusk and count_in_hand(me, EXPLORER_GUIDANCE) > 0:
            score = 60000
        elif not state.supporterPlayed and confirmed_lucario_line(opponent) and opponent.handCount >= 5:
            # Lucario's Item/evolution turn is its highest-leverage setup
            # window.  This only fires after the visible Pokémon line confirms
            # the matchup, so generic shared trainer cards cannot cause a
            # false-positive disruption turn.
            score = 360000
