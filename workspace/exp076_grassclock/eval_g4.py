...
Diagnostic already run (probe, 6 games/slot): with build_g4 the pilot reaches 3
...
build_g4: -1 Jumbo Ice Cream (dead: needs 3+ energy), -2 Lisia's Appeal,
...
def main():
    ...
    opp = EB.opponents()
    res = {a: {} for a in ARMS}
    print(f"H18 grass clock (+4 Basic Grass for Superb Scissors), n={n}/matchup, CRN, lucario_v2 pool\n")
    print(f"{'matchup':14}{'weight':>8}{'base':>9}{'g4':>9}{'delta':>9}   errors")
    ...
    json.dump({"per_matchup": res, "band": band},
              open(os.path.join(HERE, f"g4_n{n}.json"), "w"), indent=1)
